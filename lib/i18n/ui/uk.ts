...
export const uk: UiLex = {
  ...
  wiki: {
    ...
    earningsFallback: (name, range, asOf) =>
      `За оцінкою Streamer Times, дохід ${name} становить ${range} на місяць${asOf ? ` (станом на ${asOf})` : ''}, виходячи з виміряної кількості глядачів і годин в ефірі.`,
    earningsMethodology: 'Як розраховано цю оцінку',
    numbersHeading: 'У цифрах',
    numbersNote: (days) =>
      `Медіани глядачів ґрунтуються на щогодинних замірах під час ефіру; активність охоплює останні ${days} днів.`,
    numberLabel: {
      medianViewers: 'Глядачі (медіана)',
      streamsPerWeek: 'Стримів на тиждень',
      activeDays: 'Активних днів на тиждень',
      typicalLength: 'Типова тривалість',
      followerGain30: 'Нові підписники за 30 днів',
    },
    streamTimesHeading: (name) => `Коли стримить ${name}?`,
    reliabilityLabel: 'Надійність розкладу',
    reliabilityTier: { reliable: 'Надійний', medium: 'Середній', unreliable: 'Ненадійний' },
    charts: {
      weekdayHeading: 'Медіана глядачів за днями тижня',
      weekdayNote: 'Дні рахуються за календарем UTC.',
      hourHeading: 'Медіана глядачів за годинами',
      tzGroupAria: 'Часовий пояс графіка за годинами',
      yourTime: 'Ваш час',
      streamerTime: 'Час стримера',
      streamerTimeNote: "Місцевий час стримера ({tz}).",
      yourTimeNote: 'Ваш місцевий час.',
      utcNote: 'UTC.',
      legendCold: 'Мало',
      legendPrime: 'Прайм-тайм',
      legendFaded: 'блідий = мало замірів',
      weekdayAria: 'Медіана одночасних глядачів за днями тижня',
      hourAria: 'Медіана одночасних глядачів за годинами доби',
      tooltip: "{label} · медіана {median} глядачів ({samples} замірів)",
      noData: "{label} · немає даних",
      followersNow: 'зараз',
      followersLow: 'мінімум',
      followerAria: "Кількість підписників з {from} по {to}",
    },
    gamesNote: (days) => `Частка серед категоризованих стримів за останні ${days} днів.`,
    gamesColGame: 'Гра',
    gamesColShare: 'Частка',
    gamesColStreams: 'Стримів',
    gamesColRank: 'Місце',
    clipsHeading: 'Яскраві моменти',
    clipsIntro: (name) => `Найпопулярніші кліпи зі стримів ${name}.`,
    clipViews: (views) => `${views} переглядів`,
    clipAria: (title) => `Дивитися кліп «${title}» на Twitch`,
    ...
    changesHeading: 'Зміни',
    changeAdded: (label, value) => `Додано ${label}: ${value}`,
    changeChanged: (label, from, to) => `${label}: змінено з ${from} на ${to}`,
    changeRemoved: (label) => `Видалено: ${label}`,
    changeIncomeRefreshed: (from, to) => `Оцінку місячного доходу оновлено з ${from} до ${to}`,
    ...
    sectionContentStyle: 'Контент і стиль',
    sectionCommunity: 'Спільнота',
    sectionAwards: 'Нагороди та рекорди',
    timelineHeading: 'Хронологія',
    linksLabel: 'Офіційні посилання',
    ...
    disclaimer: (name) =>
      `Цей вікі-профіль укладено за відкритими джерелами. Цифри на кшталт статків і доходів є оцінками третіх сторін, а не підтвердженими даними; особиста інформація відображає те, що ${name} або авторитетні медіа оприлюднили самі.`,
    disclaimerContact:
