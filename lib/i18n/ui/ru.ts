...
export const ru: UiLex = {
  ...
  stats: {
    ...
    tzToggleYour: 'Ваше время',
    tzToggleAria: 'Показывать время в',
  ...
  wiki: {
    ...
    earningsFallback: (name, range, asOf) =>
      `По оценке Streamer Times, доход ${name} составляет ${range} в месяц${asOf ? ` (по данным за ${asOf})` : ''}, исходя из измеренного числа зрителей и часов в эфире.`,
    earningsMethodology: 'Как рассчитана эта оценка',
    numbersHeading: 'В цифрах',
    numbersNote: (days) =>
      `Медианы зрителей основаны на ежечасных замерах во время эфира; активность учитывает последние ${days} дней.`,
    numberLabel: {
      medianViewers: 'Зрители (медиана)',
      streamsPerWeek: 'Стримов в неделю',
      activeDays: 'Активных дней в неделю',
      typicalLength: 'Типичная длительность',
      followerGain30: 'Новые подписчики за 30 дней',
    },
    streamTimesHeading: (name) => `Когда стримит ${name}?`,
    reliabilityLabel: 'Надёжность расписания',
    reliabilityTier: { reliable: 'Надёжное', medium: 'Среднее', unreliable: 'Ненадёжное' },
    charts: {
      weekdayHeading: 'Медиана зрителей по дням недели',
      weekdayNote: 'Дни считаются по календарю UTC.',
      hourHeading: 'Медиана зрителей по часам',
      tzGroupAria: 'Часовой пояс графика по часам',
      yourTime: 'Ваше время',
      streamerTime: 'Время стримера',
      streamerTimeNote: "Местное время стримера ({tz}).",
      yourTimeNote: 'Ваше местное время.',
      utcNote: 'UTC.',
      legendCold: 'Мало',
      legendPrime: 'Прайм-тайм',
      legendFaded: 'бледный = мало замеров',
      weekdayAria: 'Медиана одновременных зрителей по дням недели',
      hourAria: 'Медиана одновременных зрителей по часам суток',
      tooltip: "{label} · медиана {median} зрителей ({samples} замеров)",
      noData: "{label} · нет данных",
      followersNow: 'сейчас',
      followersLow: 'минимум',
      followerAria: "Число подписчиков с {from} по {to}",
    },
    gamesNote: (days) => `Доля среди категоризированных стримов за последние ${days} дней.`,
    gamesColGame: 'Игра',
    gamesColShare: 'Доля',
    gamesColStreams: 'Стримов',
    gamesColRank: 'Место',
    clipsHeading: 'Яркие моменты',
    clipsIntro: (name) => `Самые просматриваемые клипы со стримов ${name}.`,
    clipViews: (views) => `${views} просмотров`,
    clipAria: (title) => `Смотреть клип «${title}» на Twitch`,
    ...
    changeAdded: (label, value) => `Добавлено ${label}: ${value}`,
    changeChanged: (label, from, to) => `${label}: изменено с ${from} на ${to}`,
    changeRemoved: (label) => `Удалено: ${label}`,
    changeIncomeRefreshed: (from, to) => `Оценка месячного дохода обновлена с ${from} до ${to}`,
    ...
    sectionContentStyle: 'Контент и стиль',
    sectionCommunity: 'Сообщество',
    sectionAwards: 'Награды и рекорды',
    timelineHeading: 'Хронология',
    linksLabel: 'Официальные ссылки',
    ...
    disclaimer: (name) =>
      `Этот вики-профиль составлен по открытым источникам. Цифры вроде состояния и доходов являются оценками третьих сторон, а не подтверждёнными данными; личные сведения отражают то, что ${name} или авторитетные издания сделали публичным.`,
    disclaimerContact:
