...
export const ja: UiLex = {
  ...
  wiki: {
    ...
    earningsFallback: (name, range, asOf) =>
      `Streamer Timesは、計測した同時視聴者数と配信時間をもとに、${name}の収入を月${range}${asOf ? `（${asOf}時点）` : ''}と推定しています。`,
    earningsMethodology: 'この推定の仕組み',
    numbersHeading: '数字で見る',
    numbersNote: (days) =>
      `視聴者数の中央値は配信中の1時間ごとの計測に基づき、活動指標は直近${days}日間を対象としています。`,
    numberLabel: {
      medianViewers: '視聴者数（中央値）',
      streamsPerWeek: '週あたりの配信数',
      activeDays: '週あたりの配信日数',
      typicalLength: '典型的な配信時間',
      followerGain30: '30日間のフォロワー増加',
    },
    streamTimesHeading: (name) => `${name}はいつ配信する？`,
    reliabilityLabel: 'スケジュールの信頼性',
    reliabilityTier: { reliable: '高い', medium: '普通', unreliable: '低い' },
    charts: {
      weekdayHeading: '曜日別の視聴者数中央値',
      weekdayNote: '曜日はUTCの暦に従います。',
      hourHeading: '時間帯別の視聴者数中央値',
      tzGroupAria: '時間帯グラフのタイムゾーン',
      yourTime: 'あなたの時間',
      streamerTime: '配信者の時間',
      streamerTimeNote: "配信者の現地時間（{tz}）。",
      yourTimeNote: 'あなたの現地時間。',
      utcNote: 'UTC。',
      legendCold: '少ない',
      legendPrime: 'ゴールデンタイム',
      legendFaded: '薄い = サンプルが少ない',
      weekdayAria: '曜日別の同時視聴者数中央値',
      hourAria: '時間帯別の同時視聴者数中央値',
      tooltip: "{label}・中央値{median}人（{samples}サンプル）",
      noData: "{label}・データなし",
      followersNow: '現在',
      followersLow: '最小',
      followerAria: "{from}から{to}までのフォロワー数",
    },
    gamesNote: (days) => `直近${days}日間のカテゴリ付き配信に占める割合。`,
    gamesColGame: 'ゲーム',
    gamesColShare: '割合',
    gamesColStreams: '配信数',
    gamesColRank: '順位',
    clipsHeading: '注目のクリップ',
    clipsIntro: (name) => `${name}の配信で最も視聴されたクリップ。`,
    clipViews: (views) => `${views}回視聴`,
