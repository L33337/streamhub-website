...
export const es: UiLex = {
  ...
  wiki: {
    teaserTitle: 'Wiki y datos',
    teaserSub: (name, parts) => `${parts}: el perfil wiki de ${name}`,
    breadcrumb: 'Wiki',
    ...
    factLabel: {
      ...
      net_worth_usd: 'Patrimonio (est.)',
      est_income_monthly_usd: 'Ingresos mensuales (est.)',
      career_start: 'Streamea desde',
      teams: 'Equipos y orgs',
      height_cm: 'Estatura',
    },
    relationship: {
      single: 'Soltero/a',
    ...
    ageSuffix: (age) => `${age} años`,
    estimate: 'estimación',
    asOf: (when) => `datos de ${when}`,
    sectionCareer: 'Carrera',
    sectionPersonalLife: 'Vida personal',
    sectionEarnings: 'Ingresos y patrimonio',
    sectionContentStyle: 'Contenido y estilo',
    sectionCommunity: 'Comunidad',
    sectionAwards: 'Premios y récords',
    timelineHeading: 'Cronología',
    linksLabel: 'Enlaces oficiales',
    aboutHeading: (name) => `Sobre ${name}`,
    nextStreamHeading: 'Próximo stream',
    fullSchedule: (name) => `Ver el calendario completo de ${name}`,
    sourcesHeading: 'Fuentes',
    minorNote: 'De los streamers menores de 18 años solo publicamos información sobre su carrera.',
    disclaimerHeading: 'Sobre esta página',
    disclaimer: (name) =>
      `Este perfil wiki se ha elaborado a partir de fuentes públicas. Cifras como el patrimonio o los ingresos son estimaciones de terceros, no valores verificados; los datos personales reflejan lo que ${name} o medios consolidados han hecho público.`,
    disclaimerContact:
      '¿Eres este streamer y quieres corregir o eliminar algo? Escríbenos:',
    earningsFallback: (name, range, asOf) =>
      `Streamer Times estima los ingresos de ${name} en ${range} al mes${asOf ? ` (datos de ${asOf})` : ''}, a partir de los espectadores medidos y las horas emitidas.`,
    earningsMethodology: 'Cómo se calcula esta estimación',
    numbersHeading: 'En cifras',
    numbersNote: (days) =>
      `Las medianas de espectadores salen de muestras horarias durante el directo; la actividad cubre los últimos ${days} días.`,
    numberLabel: {
      medianViewers: 'Espectadores (mediana)',
      streamsPerWeek: 'Streams por semana',
      activeDays: 'Días activos por semana',
      typicalLength: 'Duración típica',
      followerGain30: 'Seguidores nuevos, 30 días',
    },
    streamTimesHeading: (name) => `¿Cuándo streamea ${name}?`,
    reliabilityLabel: 'Fiabilidad del horario',
    reliabilityTier: { reliable: 'Fiable', medium: 'Media', unreliable: 'Poco fiable' },
    charts: {
      weekdayHeading: 'Mediana de espectadores por día',
      weekdayNote: 'Los días siguen el calendario UTC.',
      hourHeading: 'Mediana de espectadores por hora',
      tzGroupAria: 'Zona horaria del gráfico por horas',
      yourTime: 'Tu hora',
      streamerTime: 'Hora del streamer',
      streamerTimeNote: "Hora local del streamer ({tz}).",
      yourTimeNote: 'Tu hora local.',
      utcNote: 'UTC.',
      legendCold: 'Bajo',
      legendPrime: 'Prime time',
      legendFaded: 'atenuado = pocas muestras',
      weekdayAria: 'Mediana de espectadores simultáneos por día de la semana',
      hourAria: 'Mediana de espectadores simultáneos por hora del día',
      tooltip: "{label} · mediana {median} espectadores ({samples} muestras)",
      noData: "{label} · sin datos",
      followersNow: 'ahora',
      followersLow: 'mínimo',
      followerAria: "Seguidores del {from} al {to}",
    },
    gamesNote: (days) => `Porcentaje de los streams categorizados en los últimos ${days} días.`,
    gamesColGame: 'Juego',
    gamesColShare: 'Cuota',
    gamesColStreams: 'Streams',
    gamesColRank: 'Puesto',
    clipsHeading: 'Momentos destacados',
    clipsIntro: (name) => `Los clips más vistos de los streams de ${name}.`,
    clipViews: (views) => `${views} visualizaciones`,
    clipAria: (title) => `Ver el clip «${title}» en Twitch`,
    ...
    changeAdded: (label, value) => `Añadido ${label}: ${value}`,
    changeChanged: (label, from, to) => `${label} cambió de ${from} a ${to}`,
    changeRemoved: (label) => `Eliminado: ${label}`,
    changeIncomeRefreshed: (from, to) => `Ingresos mensuales estimados actualizados de ${from} a ${to}`,
    changeIncomeRemoved: 'Ingresos mensuales estimados eliminados (datos insuficientes)',
    changeSections: (sections) => `Secciones reescritas: ${sections}`,
    sectionSummary: 'Introducción',
    tocLabel: 'En esta página',
    tocAbout: 'Perfil',
    tocStreamTimes: 'Horarios',
    tocGames: 'Juegos',
    articleLanguageNote: 'Este artículo aún no está disponible en español y se muestra en inglés.',
  },
};
