...
export const pt: UiLex = {
  ...
  wiki: {
    ...
    teaserSub: (name, parts) => `${parts}: o perfil wiki de ${name}`,
    breadcrumb: 'Wiki',
    ...
    sectionContentStyle: 'Conteúdo e estilo',
    sectionCommunity: 'Comunidade',
    sectionAwards: 'Prêmios e recordes',
    timelineHeading: 'Linha do tempo',
    linksLabel: 'Links oficiais',
    ...
    earningsFallback: (name, range, asOf) =>
      `A Streamer Times estima a renda de ${name} em ${range} por mês${asOf ? ` (dados de ${asOf})` : ''}, com base nos espectadores medidos e nas horas transmitidas.`,
    earningsMethodology: 'Como essa estimativa é calculada',
    numbersHeading: 'Em números',
    numbersNote: (days) =>
      `As medianas de espectadores vêm de amostras por hora durante a live; a atividade cobre os últimos ${days} dias.`,
    numberLabel: {
      medianViewers: 'Espectadores (mediana)',
      streamsPerWeek: 'Streams por semana',
      activeDays: 'Dias ativos por semana',
      typicalLength: 'Duração típica',
      followerGain30: 'Seguidores ganhos, 30 dias',
    },
    streamTimesHeading: (name) => `Quando ${name} faz stream?`,
    reliabilityLabel: 'Confiabilidade da agenda',
    reliabilityTier: { reliable: 'Confiável', medium: 'Média', unreliable: 'Pouco confiável' },
    charts: {
      weekdayHeading: 'Mediana de espectadores por dia da semana',
      weekdayNote: 'Os dias seguem o calendário UTC.',
      hourHeading: 'Mediana de espectadores por hora',
      tzGroupAria: 'Fuso horário do gráfico por hora',
      yourTime: 'Seu horário',
      streamerTime: 'Horário do streamer',
      streamerTimeNote: "Horário local do streamer ({tz}).",
      yourTimeNote: 'Seu horário local.',
      utcNote: 'UTC.',
      legendCold: 'Baixo',
      legendPrime: 'Horário nobre',
      legendFaded: 'esmaecido = poucas amostras',
      weekdayAria: 'Mediana de espectadores simultâneos por dia da semana',
      hourAria: 'Mediana de espectadores simultâneos por hora do dia',
      tooltip: "{label} · mediana de {median} espectadores ({samples} amostras)",
      noData: "{label} · sem dados",
      followersNow: 'agora',
      followersLow: 'mínimo',
      followerAria: "Seguidores de {from} a {to}",
    },
    gamesNote: (days) => `Participação nos streams categorizados dos últimos ${days} dias.`,
    gamesColGame: 'Jogo',
    gamesColShare: 'Participação',
    gamesColStreams: 'Streams',
    gamesColRank: 'Posição',
    clipsHeading: 'Momentos marcantes',
    clipsIntro: (name) => `Os clipes mais vistos dos streams de ${name}.`,
    clipViews: (views) => `${views} visualizações`,
    clipAria: (title) => `Assistir ao clipe "${title}" na Twitch`,
    recapsHeading: 'Nos resumos',
    recapsIntro: (name) =>
      `Edições dos nossos resumos semanais e mensais em que ${name} foi um dos protagonistas.`,
    historyHeading: 'Histórico',
    historyNote:
      'Mês a mês, conforme medimos: streams, horas, jogo principal e audiência. Meses com uma mudança notável recebem um resumo curto.',
    historyColMonth: 'Mês',
    historyColStreams: 'Streams',
    historyColHours: 'Horas',
    historyColTopGame: 'Jogo principal',
    ...
    changeAdded: (label, value) => `Adicionado ${label}: ${value}`,
    changeChanged: (label, from, to) => `${label} alterado de ${from} para ${to}`,
    changeRemoved: (label) => `Removido ${label}`,
    changeIncomeRefreshed: (from, to) => `Renda mensal estimada atualizada de ${from} para ${to}`,
    changeIncomeRemoved: 'Renda mensal estimada removida (dados insuficientes)',
    changeSections: (sections) => `Seções reescritas: ${sections}`,
    sectionSummary: 'Introdução',
    tocLabel: 'Nesta página',
    tocAbout: 'Perfil',
    tocStreamTimes: 'Horários',
    tocGames: 'Jogos',
    articleLanguageNote: 'Este artigo ainda não está disponível em português e é exibido em inglês.',
  },
};
