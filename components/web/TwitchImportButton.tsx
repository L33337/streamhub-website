...
export function TwitchImportButton() {
  ...
  // A declined/failed "Connect Twitch" round-trip returns here as
  // ?connect_error=… (the return page can't reach this component's state across
  // the full-page OAuth navigation). Derive it during render — an inline
  // pre-flight error (connectError state) takes precedence over the URL one.
  const roundTripErrorCode = searchParams.get('connect_error');
  const displayedError =
    connectError ??
    (roundTripErrorCode ? connectErrorCopy(roundTripErrorCode) : null);
  ...
  function handleReconnect() {
    ...
    // Navigates to Twitch and returns here via /auth/twitch-import with a fresh
    // provider token captured into localStorage. Only the pre-flight error
    // case resolves.
    const { error } = startTwitchFollowImport(pathname);
    if (error) {
    ...
    return (
      ...
      >
        <p className="text-sm text-text-secondary">
          Connect your Twitch account to bring your follows over. We only read
          who you follow — your Streamer Times sign-in stays as it is.
        </p>
        {displayedError && (
          <div
            role="alert"
